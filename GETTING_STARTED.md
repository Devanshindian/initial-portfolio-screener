...
1. In the VS Code left panel, find the folder named **`companies`**. Right-click it and choose **New Folder**. Name it after the company, with no spaces, for example **`Popees`**.
2. Now copy the company's documents into that new folder. The simplest way: open the `companies/Popees` folder on your computer (in Finder on Mac or File Explorer on Windows) and drag the files in. You can also drag files straight onto the `companies/Popees` folder inside VS Code.
3. Helpful documents to include (whatever you have):
...
**Worked example, already filled in for Popees** (this is exactly what a finished prompt looks like):

```
Run a Nandan initial check on Popees.

Documents in folder companies/Popees/
- Audited annual reports: FY25 and FY24
- MIS financials: under the name "Projections 5 Years" has provisional till 31st March 2026 (all 12 months)
...
Deal terms (if known):
- Ask: Cr (check pitch deck)
- Tenure: __ months (check pitch deck if given)
- Securities offered: not known
